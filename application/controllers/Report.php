...
public function cetak(){
    ...
    $kode = $this->input->get('kode', true);
        ...
        if ($produk==1) {
            $data['cek'] =  $this->db->query("SELECT * from v_report_produk WHERE kode_produk LIKE '%{$kode}%' and DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
            $data['retur'] =  $this->db->query("SELECT * from v_retur WHERE kode_produk LIKE '%{$kode}%' and DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
        }else if($penjualan==1){
            $data['cek'] =  $this->db->query("SELECT * from v_report_produk WHERE no_faktur LIKE '%{$kode}%' and DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
            $data['retur'] =  $this->db->query("SELECT * from v_retur WHERE no_faktur LIKE '%{$kode}%' and DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
        }else{
            $data['cek'] =  $this->db->query("SELECT * from v_report_produk WHERE kode_produk LIKE '%{$kode}%' and DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
            $data['retur'] =  $this->db->query("SELECT * from v_retur WHERE kode_produk LIKE '%{$kode}%' and DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
        }
        ...
        $data['cek'] =  $this->db->query("SELECT * from v_report_produk WHERE DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
        $data['retur'] =  $this->db->query("SELECT * from v_retur WHERE DATE(tanggal_penjualan) = '".$tanggal_mulai."'")->result();
    }
}
       
        $oel = '
        
    <style>table,p{font-family: Arial, Helvetica, sans-serif;border-collapse: collapse;font-size:12px;}</style>
    <table style="width:100%;">
    <tr>
    
    <td colspan="3" style="text-align: center;font-size:14px !important;"><h3>Report Transaksi Penjualan</h3></td>
